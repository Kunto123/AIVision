...
- **Admin** — semua tab (TEACH, HISTORY, SETTINGS, Akun, I/O Settings).
...
Kamera (device, resolusi, FPS, exposure) · PLC (port, baudrate) · mode & timing inferensi · retensi history · Flask API · PostgreSQL · bahasa. **ROI dan engine/threshold model diatur per-template di tab TEACH**, bukan di sini.
