...
Membaca blok special relay M8000+ untuk memastikan PLC menjawab. `fxplc` ikut ter-install dari `requirements.txt` (arsip .zip GitHub).

Scan/deteksi coil aktif ada di **Settings → PLC** (Scan range).
...
Backbone pretrained (timm/HuggingFace untuk PatchCore/EfficientAd, GitHub untuk YOLO) diunduh otomatis saat training pertama dan di-cache. Butuh akses internet ke `huggingface.co` + `github.com` saat itu.
...
Monitoring: latensi inferensi + FPS kamera tampil di layar RUN. RAM/CPU proses lewat Task Manager Windows. Detail runtime & log per subsistem di `data\logs\*.log`.

## Deploy

Aplikasi jalan dari source (tidak dibungkus exe). Di PC edge: salin folder proyek, jalankan `setup.bat` lalu `run.bat`. Butuh internet saat setup pertama (install deps + unduh backbone pretrained saat training pertama).
